...
async def query_auto(
    ...
    try:
        ...
        raw_sql_response = await sql_generator.generate_sql(effective_question)
...
async def convert_nl_to_sql_memory(
    request: NL2SQLRequest,
    db: Session = Depends(get_db)
):
    """Convert natural language question to SQL and execute it with memory"""
    try:
        # Step 1: Generate SQL from natural language
        logger.info(f"Converting question to SQL: {request.question}")
        raw_sql_response = await sql_generator.generate_sql(request.question)
